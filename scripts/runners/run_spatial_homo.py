...
def make_diff_func(p, q):
    @njit
    def diff_func(pos):
        # D(x) = -p * cos(0.2* pi * x) + q
        D = -p * math.cos(0.2 * math.pi * pos) + q
        
        # Derivative: dD/dx = 0.2 * p * pi * sin(0.2 * pi * x)
        dD_dx = 0.2 * p * math.pi * math.sin(0.2 * math.pi * pos)
        
        return D, dD_dx  # Return both!
    return diff_func
...
def run_save_spatial(num, t_f, tau, ls, sigmas, diffusions, c_a, c_b, box_shape, result_dir, p, q, hetero_diff=False):
    ...
    for i in range(start_index, start_index + NUM_RUNS_TO_DO):
        ...
        if hetero_diff:
            ...
            -> D(x) = -p * cos(pi*x) + q, where p, q are constants.
            ...
            p = 0 # when the diffusion coefficient is constant in the box, 
            q = 0 # set p and q to 0 to distinguish from the heterogeneous Diffusions in metadata.
